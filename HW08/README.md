...
_* важливий момент: контейнер Jenkins має знаходитись в мережі відмінній від мережі за замовчуванням, яка не дозволяє автоматично розв’язку DNS; у моєму випадку я стоврюю окрему мережу `dev_net`
...
Або якщо ви лінивий та працюєте на Linux, то можна спростити, отримавши пароль одарзу у буфер обміну однією командою:
...
_Для підвищення безпеки використаємо шифрування приватного ключа, вставивши пароль під час створення ключа.
Пароль можна сгенерувати, наприклад, командою `pwgen -y 16`. Цей пароль рекомендую зберегти у вашому сховищі паролей, він може знадобитися у майбутньому._
```bash
...
### Розгортання агента
Розгорнемо додатову віртуальну машину `dev-worker01` за допомогою Terraform, використовуючи конфігурацію із [четвертої роботи](../HW04/README.md).
...
Основне завдання ролі встановлює середовище Java, необхідне для функціонування Jenkins, створює робочий каталог, а також інсталює публічний ключ, створений нами на поередніх етапах, для доступу Jenkins до агентів.
...
* **Labels:** `dev libvirt` (це дві мітки із назвами _dev_ та _libvirt_, які потім можна використувати при створенні piplines)
...
Оскільки ми плануємо збирати образи Docker, то нам потрібна кінцева точка, куди вони будуть зберігатися. Зазвичай для таких цілей використовується публічне сховище накшалт DockerHub, проте я для екпериментів надаю перевагу локальним ресурсам.
